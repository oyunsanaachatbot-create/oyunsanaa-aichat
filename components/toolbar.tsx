...
const PureToolbar = ({
  ...
  // ✅ wrapper: Chat товч + Tools товч хоёрыг зэрэг байрлуулна
  return (
    <TooltipProvider delayDuration={0}>
      <div className="absolute right-6 bottom-6 z-50 flex flex-col items-end gap-3">
        {/* ✅ Chat button (mobile only, 1 tap) */}
        {onToggleMobileChat && (
          <motion.button
            type="button"
        ...
        {/* ✅ Existing tools bubble */}
        <motion.div
          animate={
            isToolbarVisible
              ? selectedTool === "adjust-reading-level"
                ? { opacity: 1, y: 0, height: 6 * 43, transition: { delay: 0 }, scale: 0.95 }
                : {
                    opacity: 1,
                    y: 0,
          ...
          className="flex cursor-pointer flex-col justify-end rounded-full border bg-background p-1.5 shadow-lg"
