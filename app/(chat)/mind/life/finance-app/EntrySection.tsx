...
export function EntrySection(props: {
  ...
  useEffect(() => {
    if (type === "income") {
      ...
      setCategory("debt");
      setSubCategory("debt_borrow"); // default
    } else {
      if (category === "income" || category === "debt") setCategory("food");
      setSubCategory("");
    }
  ...
  useEffect(() => {
    // category өөрчлөгдөхөд sub reset (income/debt үед default-той байж болно)
    if (type === "income") setSubCategory("");
    else if (type === "debt") setSubCategory((v) => v || "debt_borrow");
    else setSubCategory("");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [category]);
  ...
  const handleAddClick = async () => {
    ...
    setAmount("");
    setNote("");
    setSubCategory(type === "debt" ? "debt_borrow" : "");
  };

  return (
    <section className="mt-8 space-y-3">
      {/* ✅ Тайлан товчтой ижил хэв маягтай toggle */}
      <button
        type="button"
      ...
      {showEntry && (
        <div className="grid md:grid-cols-[minmax(0,1.3fr)_minmax(0,1.7fr)] gap-5">
          <div className="space-y-4">
            <div className="rounded-2xl border border-white/25 bg-white/10 px-4 py-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {/* ✅ Категори: income/debt үед fixed */}
                <div className="flex flex-col gap-1 text-xs col-span-2">
                  ...
                  <select
                    ...
                    className="rounded-xl border border-white/25 bg-white/10 px-3 py-1.5 text-sm text-slate-50 outline-none focus:border-white/60"
                    disabled={type === "income" || type === "debt"}
                  >
                  ...
                  {(type === "income" || type === "debt") && (
                    <p className="text-[10px] text-slate-300">
                      {type === "income"
                        ? "Орлого нь нэг л категори (Орлого). Харин “төрөл”-өө доороос сонгоно."
                        : "Өр/Зээл нь нэг л категори (Өр/Зээл). Доороос “авсан/төлсөн”-өө сонгоно."}
                    </p>
                  )}
                </div>
              </div>
          ...
          {/* (Баруун талд “сүүлийн гүйлгээнүүд”-ийг FinanceAppClient дээр зурна) */}
          <div className="rounded-2xl border border-white/25 bg-white/10 px-4 py-4 text-[11px] text-slate-200">
            <p className="text-slate-100 font-medium mb-1">Зөвлөгөө</p>
            <ul className="list-disc ml-5 space-y-1">
              <li>Цалин авсан бол: <b>Орлого</b> → төрөл: <b>Цалин</b></li>
              <li>Зээл авсан бол: <b>Өр/Зээл</b> → <b>Зээл авсан</b></li>
              <li>Зээл төлсөн бол: <b>Өр/Зээл</b> → <b>Зээл төлсөн</b></li>
              <li>Хоол авсан бол: <b>Зарлага</b> → категори: <b>Хоол</b> → дэд төрөл сонгоно</li>
            </ul>
          </div>
        </div>
      )}
