...
function summaryLine(level: Level, score: number) {
  ...
  if (level === "Orange") return `Өнөөдөр ачаалалтай өдөр байсан бололтой 🫶 (${score}/100)`;
  return `Өнөөдөр хүнд өдөр байсан бололтой —Өөрийгөө буруутгах хэрэггүй✨ (${score}/100)`;
}
...
function detailLine(level: Level) {
  ...
  if (level === "Yellow") return "Хэлбэлзэл бага зэрэг байна. Өөртөө жижигхэн амралт өгвөл сайн шүү.";
  if (level === "Orange") return "Ачаалал мэдрэгдэж байна. Өөртаа анхаарал хандуулаарай.";
  return "Хүнд мэдрэмж давамгайлж байна. чи ганцаараа биш шүү— хүсвэл би ярилцахад үргэлж бэлэн байна";
}
...
function finishWarm(finishText: string) {
  ...
  return m[finishText] 
    ?? `"${finishText}" гэж хэлсэн чинь өөрөө хүч.` 
    ?? "";
}
