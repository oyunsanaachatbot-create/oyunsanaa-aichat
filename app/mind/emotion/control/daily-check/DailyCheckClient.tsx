...
const STEPS: Step[] = [
  ...
  },
  {
    ...
    desc: "Нөлөө ихсэх тусам өнөөдөр ачаалал өндөр байна гэж үзнэ.",
    choices: [
  ...
  },
  {
    ...
    desc: "Өнөөдөр өөрийгөө яаж авч явах вэ гэдгийг сонгоё.",
    choices: [
  ...
  },
  {
    ...
    desc: "Энэ нь нэр өгөхөд тусална (оноонд бага нөлөөлнө).",
    choices: [
...
function summaryLine(level: Level, score: number) {
  if (level === "Green") return `Өнөөдөр нэлээн тогтвортой байна 🌿 (${score}/100)`;
  if (level === "Yellow") return `Өнөөдөр боломжийн байна 👏 (${score}/100)`;
  if (level === "Orange") return `Өнөөдөр ачаалал мэдрэгдэж байна 🫶 (${score}/100)`;
  return `Өнөөдөр нэлээн хүнд байна. Гэхдээ чи ганцаараа биш 🤍 (${score}/100)`;
}

function detailLine(level: Level) {
  if (level === "Green") return "Сэтгэл-биеийн тэнцвэр сайн байна. Энэ байдлаа жижиг дадлаар бататгаарай.";
  ...
  return "Хүнд мэдрэмж давамгайлж байна. Өөрийгөө буруутгах хэрэггүй — амьсгал, ус, жижиг амралт хангалттай.";
}

function adviceLine(level: Level, needText: string) {
  // need сонголт дээр тулгуурлаад зөвлөгөөг өөрчилнө
  const base =
    level === "Red"
      ? "Өнөөдөр хамгийн бага ачаалалтай горим сонгоё."
      : level === "Orange"
      ? "Өнөөдөр өөртөө зөөлөн хандах өдөр."
      : level === "Yellow"
      ? "Жижигхэн тохируулга хийвэл бүр дээрдэнэ."
      : "Сайн байдлаа 1 жижиг үйлдлээр бататгая.";

  const byNeed: Record<string, string> = {
    "Амрах": "10–20 минут нүд амраагаад, ус уугаад, биеэ сулла.",
    "Тайвшрах": "4–4–4 амьсгал (4 сек авах, 4 барих, 4 гаргах) 3 удаа.",
    "Хөдөлгөөн хийх": "5–10 минут алхах/сунгалт хийхэд тархи тайвширдаг.",
    "Хүнтэй холбогдох": "Нэг хүн рүү 1 мөр мессеж илгээ: “Надтай жаахан ярилцаж болох уу?”",
    "Ганцаараа байх": "Чимээгүй 10 минут. Дараа нь 1 жижиг зүйл хий (ус/шүршүүр/цэгцлэх).",
  };

  const extra = byNeed[needText] ?? "Өнөөдөр өөртөө 1 жижиг тус хүргэ.";
  return `${base} ${extra}`;
}

function warmClosing(identityText: string, finishText: string) {
  const parts: string[] = [];
  if (identityText) parts.push(`Чи өөрийгөө “${identityText}” гэж харж чадсан байна.`);
  if (finishText) parts.push(`Өнөөдрийн үг: “${finishText}”.`);
  parts.push("Хэцүү байвал надтай ярилцаж болно шүү — чи ганцаараа биш 🤍");
  return parts.join(" ");
}
...
export default function DailyCheckClient() {
  ...
  const newKey = searchParams.get("new"); // menu дээрээс ?new=1 орж ирвэл reset
  ...
  // ✅ menu дээрээс ?new=1 орж ирвэл шууд шинэ тест
  useEffect(() => {
    if (newKey) resetTest();
  ...
  async function saveToSupabase() {
    ...
    // ✅ required singles UI дээр шалгаад 500-ийг тасална
    const requiredSingles = ["mood", "impact", "body", "energy", "need", "color", "finish"];
    for (const k of requiredSingles) {
      const v = answers?.[k]?.[0];
      if (!v) {
        setErr(`“${k}” асуулт хоосон байна. Буцаад сонгоорой.`);
        return;
      }
    }
        ...
        <section className={styles.card}>
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    else toggleMulti(step.id, c.id, (step as any).maxPick);
                  }}
                >
          ...
          {result ? (
            <div className={styles.resultCard}>
              ...
              {/* ✅ need дээр тулгуурласан зөвлөгөө */}
              {needText ? <div className={styles.advice}>{adviceLine(result.level, needText)}</div> : null}

              {/* ✅ identity + finish ашиглаж дулаахан хаалт */}
              <div className={styles.oyLine}>{warmClosing(identityText, finishText)}</div>
            </div>
          ) : null}
