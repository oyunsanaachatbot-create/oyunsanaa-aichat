...
export async function GET(
  ...
  if (program.audience === "ORGANIZATION") {
    ...
    if (
      !canAccessOrganizationProgram(
        access,
        program.organizationRoles,
        program.organizationDurationMonths
      )
    )
  ...
  if (!video || video.status !== "READY" || !referenced)
    return Response.json({ error: "video_not_ready" }, { status: 403 });
  const libraryId = process.env.BUNNY_STREAM_LIBRARY_ID;
  const tokenKey =
    process.env.BUNNY_STREAM_TOKEN_KEY || process.env.BUNNY_STREAM_API_KEY;
  const embedHost =
    process.env.BUNNY_STREAM_EMBED_HOST || "iframe.mediadelivery.net";
  if (!libraryId || !tokenKey)
    return Response.json({ error: "video_not_configured" }, { status: 503 });
  ...
  return Response.json(
    { videoId, token, expires, embedUrl },
